...
class TursoEvaluator:
    ...
    def _init_graphmem(self):
        ...
        config = MemoryConfig(
            ...
            # TURSO backend - no external servers!
    ...
    def run_evaluation(
        ...
        try:
            gm.save()
            print(f"   → Memory saved to: {self.turso_db_path}")
        except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="GraphMem Evaluation with Turso Backend")
    ...
    evaluator.save_results(os.path.join(output_dir, "eval_results_turso.json"))
