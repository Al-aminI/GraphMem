...
% Evolution Layer
\node[component, below=0.8cm of graph] (importance) {Importance\\Scoring};
\node[component, below=0.8cm of importance] (decay) {Memory\\Decay};
\node[component, below=0.8cm of decay] (consolidate) {LLM\\Consolidation};

% Retrieval Layer
\node[component, below=0.8cm of consolidate] (retrieval) {Hybrid Retrieval};

% Storage Layer
\node[storage, left=1.1cm of importance] (neo4j) {Neo4j\\(Graph DB)};
\node[storage, left=1.1cm of decay] (redis) {Redis\\(Cache)};
\node[storage, left=1.1cm of consolidate] (embed) {Embeddings\\(Vector)};

% Output
\node[process, right=3.5cm of retrieval] (output) {Context\\Assembly};
...
\begin{scope}[on background layer]
    % Ingest Pipeline: input + top processing
    \node[draw=gray!50, dashed, rounded corners=5pt, fill=blue!5,
          fit=(input)(extract)(resolve)(graph),
          inner sep=0.2cm,
          label={[font=\scriptsize\sffamily]above:Ingest Pipeline}] {};

    % Evolution: vertical chain under graph
    \node[draw=gray!50, dashed, rounded corners=5pt, fill=orange!5,
          fit=(importance)(decay)(consolidate),
          inner sep=0.2cm,
          label={[font=\scriptsize\sffamily]right:Evolution}] {};

    % Storage: column of DBs to the left
    \node[draw=gray!50, dashed, rounded corners=5pt, fill=green!5,
          fit=(neo4j)(redis)(embed),
          inner sep=0.2cm,
          label={[font=\scriptsize\sffamily]left:Storage}] {};

    % Retrieval / Output region (optional, if desired)
    %\node[draw=gray!50, dashed, rounded corners=5pt, fill=purple!5,
    %      fit=(retrieval)(output)(llm),
    %      inner sep=0.3cm,
    %      label={[font=\scriptsize\sffamily]below:Retrieval & Generation}] {};
\end{scope}
...
\begin{axis}[
    ...
    ymin=0,
    ymax=800,
    bar width=18pt,        % narrower bars
    enlarge x limits=0.35, % add horizontal padding around bars
    nodes near coords,
